...
public class OrderServiceImpl implements OrderService {
    ...
    @Autowired
    private StorePresaleService presaleService;
    ...
    public Boolean refundApplyTask(List<OrderRefundApplyRequest> applyList) {
        ...
        if (orderList.size() > 0) {
            return storeOrderService.updateBatchById(orderList);
        }
        return true;
     ...
     */
    @Override
    public Object expressOrder(String orderId, String type) {
        ...
        LogisticsResultVo expressInfo = logisticsService.info(existOrder.getDeliveryId(), type, Optional.ofNullable(existOrder.getDeliveryCode()).orElse(""), storeOrderPram.getUserPhone());
    ...
    public MyRecord preOrder(PreOrderRequest request) {
        ...
        if (ObjectUtil.isNotNull(userAddress)) {
            ...
            orderInfoVo.setPostalCode(userAddress.getPostalCode());
    ...
    public MyRecord createOrder(CreateOrderRequest request) {
        ...
        // 检测是否可以下单
        if (orderInfoVo.getSeckillId() != null && orderInfoVo.getSeckillId() > 0) {
            LambdaQueryWrapper<StoreOrder> countWrapper = new LambdaQueryWrapper<>();
            LocalDate now = LocalDate.now();
            countWrapper.between(StoreOrder::getCreateTime, new Date(now.atStartOfDay(GMT).toEpochSecond() * 1000), new Date(now.plusDays(1L).atStartOfDay(GMT).toEpochSecond() * 1000));
            countWrapper.eq(StoreOrder::getUid, userService.getUserId());
            countWrapper.ne(StoreOrder::getRefundStatus, 2);
            int count = storeOrderService.count(countWrapper);
            // 每日最大秒杀次数
            String maxString = systemConfigService.getValueByKey(SysConfigConstants.MAX_SECKIL_NUMBER);
            if (StrUtil.isNotBlank(maxString)) {
                int max = Integer.parseInt(maxString);
                if (count >= max && max != 0) {
                    throw new CrmebException("今日秒杀次数已大上限！");
                }
            }
        }
...
//            if(!request.getPayType().equals(PayConstants.PAY_TYPE_KAB_PAY)){
...
//            }
        ...
        if (request.getShippingType() == 1) { // 快递配送
            ...
            request.setPostalCode(userAddress.getPostalCode());
        ...
        //更新预售用户订单号
        if (ObjectUtil.isNotNull(orderInfoVo.getPresaleId()) && orderInfoVo.getPresaleId() > 0) {
            presaleUserService.updateOrderId(orderInfoVo.getPresaleId(),orderNo);
        }
        ...
        storeOrder.setPostalCode(request.getPostalCode());
        ...
        storeOrder.setPresaleId(orderInfoVo.getPresaleId());
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (storeOrder.getSeckillId() > 0) {// 秒杀扣库存
            ...
            } else if (storeOrder.getPresaleId() > 0) {// 预售扣库存
                MyRecord skuRecord = skuRecordList.get(0);
                // 预售商品扣库存
                presaleService.operationStock(skuRecord.getInt("activityId"), skuRecord.getInt("num"), "sub");
                // 预售商品规格扣库存
                storeProductAttrValueService.operationStock(skuRecord.getInt("activityAttrValueId"), skuRecord.getInt("num"), "sub", Constants.PRODUCT_TYPE_PRESALE);
                // 普通商品口库存
                storeProductService.operationStock(skuRecord.getInt("productId"), skuRecord.getInt("num"), "sub");
                // 普通商品规格扣库存
                storeProductAttrValueService.operationStock(skuRecord.getInt("attrValueId"), skuRecord.getInt("num"), "sub", Constants.PRODUCT_TYPE_NORMAL);
                ...
                // 商品规格表扣库存
                storeProductAttrValueService.operationStock(skuRecord.getInt("attrValueId"), skuRecord.getInt("num"), "sub", Constants.PRODUCT_TYPE_COMPONENT);
                // 视频商品规格表扣库存
                componentProductSkuService.operationStock(skuRecord.getInt("skuId"), skuRecord.getInt("num"), "sub");
            } else { // 普通商品
    ...
    private List<MyRecord> validateProductStock(OrderInfoVo orderInfoVo, User user) {
        ...
        if (orderInfoVo.getPresaleId() > 0) {
            // 预售部分判断
            Integer presaleId = orderInfoVo.getPresaleId();
            OrderInfoDetailVo detailVo = orderInfoVo.getOrderDetailList().get(0);
            StorePresale storePresale = presaleService.getByIdException(presaleId);
            StoreProductAttrValue seckillAttrValue = storeProductAttrValueService.getByIdAndProductIdAndType(detailVo.getAttrValueId(), presaleId, Constants.PRODUCT_TYPE_PRESALE);
            if (ObjectUtil.isNull(seckillAttrValue)) {
                throw new CrmebException("预售商品规格不存在");
            }
            MyRecord seckillRecord = commonValidatePresale(storePresale, seckillAttrValue, user, detailVo.getPayNum());
            // 主商品sku
            StoreProductAttrValue productAttrValue = seckillRecord.get("productAttrValue");

            MyRecord record = new MyRecord();
            record.set("activityId", presaleId);
            record.set("activityAttrValueId", seckillAttrValue.getId());
            record.set("productId", storePresale.getProductId());
            record.set("attrValueId", productAttrValue.getId());
            record.set("num", detailVo.getPayNum());
            recordList.add(record);
            return recordList;
    ...
    private OrderInfoVo validatePreOrderRequest(PreOrderRequest request, User user) {
        ...
        if (request.getPreOrderType().equals("buyNow")) {// 立即购买
            ...
            if (detailRequest.getSeckillId() > 0) {// 秒杀
                ...
                detailVoList.add(validatePreOrderCombination(detailRequest, user, detailRequest.getPinkId()));
                orderInfoVo.setCombinationId(detailRequest.getCombinationId());
                orderInfoVo.setPinkId(detailRequest.getPinkId());
            }else if (detailRequest.getPresaleId() > 0) {// 预售
                detailVoList.add(validatePreOrderPresale(detailRequest, user));
                orderInfoVo.setPresaleId(detailRequest.getPresaleId());
            } else  {
                // 普通商品
                ...
                detailVo.setProductNameEn(storeProduct.getStoreNameEn());
                detailVo.setProductNameMy(storeProduct.getStoreNameMy());
     ...
     * @return OrderInfoDetailVo
     */
    private OrderInfoDetailVo validatePreOrderPresale(PreOrderDetailRequest detailRequest, User user) {
        Integer presaleId = detailRequest.getPresaleId();
        StorePresale storeSeckill = presaleService.getByIdException(presaleId);
        if (storeSeckill.getIsShow().equals(0)) {
            throw new CrmebException("预售商品已关闭");
        }
        StoreProductAttrValue seckillAttrValue = storeProductAttrValueService.getByIdAndProductIdAndType(detailRequest.getAttrValueId(), presaleId, Constants.PRODUCT_TYPE_PRESALE);
        if (ObjectUtil.isNull(seckillAttrValue)) {
            throw new CrmebException("预售商品规格不存在");
        }
        commonValidatePresale(storeSeckill, seckillAttrValue, user, detailRequest.getProductNum());
    ...
    private MyRecord commonValidatePresale(StorePresale storeSeckill, StoreProductAttrValue seckillAttrValue, User user, Integer productNum) {
        ...
        if (ObjectUtil.isNull(productAttrValue)) {
            throw new CrmebException("预售主商品规格不存在");
        }
        if (productAttrValue.getStock() <= 0 || productNum > productAttrValue.getStock()) {
            throw new CrmebException("预售主商品规格库存不足");
        }
...
//        }
        ...
        // 根据用户和预售信息查询当天订单判断订单数量
        List<StoreOrder> userCurrentDaySecKillOrders = storeOrderService.getUserCurrentPresaleOrders(user.getUid(), storeSeckill.getId());
        if (CollUtil.isNotEmpty(userCurrentDaySecKillOrders)) {
            ...
            if (unPayOrders.size() > 0) {
                throw new CrmebException("您有预售待支付订单，请支付后再购买");
            }
        }
     ...
     * @return OrderInfoDetailVo
     */
    private OrderInfoDetailVo validatePreOrderCombination(PreOrderDetailRequest detailRequest, User user, Integer pinkId) {
        // 拼团部分判断
        ...
        detailVo.setProductNameEn(storeCombination.getTitleEn());
        detailVo.setProductNameMy(storeCombination.getTitleMy());
        detailVo.setAttrValueId(combinationAttrValue.getId());
        detailVo.setSku(combinationAttrValue.getSuk());
        // 自己开团，使用开团价格
        if (pinkId == 0) {
            detailVo.setPrice(combinationAttrValue.getKtPrice());
        } else {
            StorePink pink = storePinkService.getById(pinkId);
            // 系统团使用系统团价格
            if (pink.getIsSystem()) {
                if (combinationAttrValue.getCtPrice() != null && combinationAttrValue.getCtPrice().compareTo(BigDecimal.ZERO) > 0) {
                    detailVo.setPrice(combinationAttrValue.getCtPrice());
                } else {
    ...
    private List<OrderInfoDetailVo> validatePreOrderAgain(PreOrderDetailRequest detailRequest, User user) {
        ...
        infoVoList.forEach(e -> {
            ...
            tempDetailVo.setProductNameEn(storeProduct.getStoreNameEn());
            tempDetailVo.setProductNameMy(storeProduct.getStoreNameMy());
    ...
    private ComputedOrderPriceResponse computedPrice(OrderComputedPriceRequest request, OrderInfoVo orderInfoVo, User user) {
        ...
        priceResponse.setProTotalFee(orderInfoVo.getProTotalFee());
        if (!request.getUseIntegral() || user.getIntegral() <= 0) {// 不使用金豆
            priceResponse.setDeductionPrice(BigDecimal.ZERO);
            priceResponse.setSurplusIntegral(user.getIntegral());
    ...
    @Override
    public CommonPage<StorePinkResponse> combinationOrderList(Integer status, Boolean isWinner, PageParamRequest pageRequest) {
        Integer userId = userService.getUserIdException();

        List<StorePink> StorePinkList = storeOrderService.getUserCombinationOrderList(userId, status, isWinner, pageRequest);
        CommonPage<StorePink> storePinkCommonPage = CommonPage.restPage(StorePinkList);
        List<StorePinkResponse> responseList = CollUtil.newArrayList();
        for (StorePink storePink : StorePinkList) {
            StorePinkResponse infoResponse = new StorePinkResponse();
            BeanUtils.copyProperties(storePink, infoResponse);
            // 订单详情对象列表
            List<StoreOrderInfo> orderInfoList = storeOrderInfoService.getListByOrderNo(storePink.getOrderId());
            List<OrderInfoResponse> infoResponseList = CollUtil.newArrayList();
            orderInfoList.forEach(e -> {
                OrderInfoResponse orderInfoResponse = new OrderInfoResponse();
                orderInfoResponse.setStoreName(e.getProductName());
            ...
            infoResponse.setOrderInfos(infoResponseList);
            StoreOrder storeOrder = storeOrderService.getByOderId(storePink.getOrderId());
            if (storeOrder != null) {
                infoResponse.setOrderStatus(storeOrder.getStatus());
                infoResponse.setRefundStatus(storeOrder.getRefundStatus());
            }
            responseList.add(infoResponse);
        }
        CommonPage<StorePinkResponse> detailPage = CommonPage.restPage(responseList);
        BeanUtils.copyProperties(storePinkCommonPage, detailPage, "list");
        return detailPage;
    }

    @Override
    public List<Map<String, Object>> combinationOrderCount() {
        Integer userId = userService.getUserIdException();
        QueryWrapper<StorePink> queryWrapper = new QueryWrapper<>();
        queryWrapper.select(" cast(status AS CHAR) AS status, count(*) AS count");
        queryWrapper.eq("uid", userId);
        queryWrapper.groupBy("status");
        return storePinkService.listMaps(queryWrapper);
    }
}
