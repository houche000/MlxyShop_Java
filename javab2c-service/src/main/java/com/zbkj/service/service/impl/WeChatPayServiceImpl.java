...
public class WeChatPayServiceImpl implements WeChatPayService {
    ...
    public Map<String, String> unifiedRecharge(UserRecharge userRecharge, String clientIp) {
        ...
        String channel = userRecharge.getRechargeType();
        if (channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_PUBLIC)) {// 公众号
            userToken = userTokenService.getTokenByUserId(userRecharge.getUid(), 1);
        }
        if (channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_PROGRAM)) {// 小程序
            userToken = userTokenService.getTokenByUserId(userRecharge.getUid(), 2);
        }
        if (channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_H5)) {// H5
            userToken.setToken("");
        }
        if (channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_APP_IOS)) {// app ios
            userToken = userTokenService.getTokenByUserId(userRecharge.getUid(), 5);
        }
        if (channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_APP_ANDROID)) {// app android
            userToken = userTokenService.getTokenByUserId(userRecharge.getUid(), 6);
        }
        if (!channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_H5) && !channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_APP_IOS) && !channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_APP_ANDROID)) {
        ...
        if (channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_H5)) {// H5,使用公众号的
            appId = systemConfigService.getValueByKeyException(Constants.CONFIG_KEY_PAY_WE_CHAT_APP_ID);
            mchId = systemConfigService.getValueByKeyException(Constants.CONFIG_KEY_PAY_WE_CHAT_MCH_ID);
            signKey = systemConfigService.getValueByKeyException(Constants.CONFIG_KEY_PAY_WE_CHAT_APP_KEY);
        }
        if (channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_APP_IOS) || channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_APP_ANDROID)) {// H5,使用公众号的
        ...
        if (channel.equals(PayConstants.PAY_CHANNEL_WE_CHAT_H5)) {
