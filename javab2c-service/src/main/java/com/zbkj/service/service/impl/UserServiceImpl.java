...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    public PageInfo<UserResponse> getList(UserSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        // 过滤机器人
        // lambdaQueryWrapper 为无效代码，过滤代码已写入sql
        lambdaQueryWrapper.ne(User::getUserType, Constants.USER_LOGIN_TYPE_BOT);
        ...
        for (User user : userList) {
            ...
            userExInfo.eq(UserExtractInfo::getUid,user.getUid());
            List<UserExtractInfo> list =  userExtractInfoService.list(userExInfo);
            if (list.size() != 0) {
                userResponse.setIcNumber(list.get(0).getIcNumber());
            }
            ...
            queryWrapper.eq(UserExtractInfo::getUid, userResponse.getUid());
            queryWrapper.eq(UserExtractInfo::getExtractType, "bank");
            UserExtractInfo extractInfo = extractInfoService.getOne(queryWrapper);
            userResponse.setExtractInfo(extractInfo);
            //取消脱敏
     ...
     */
    @Override
    public PageInfo<UserResponse> listBot(UserSearchRequest request, PageParamRequest pageParamRequest) {
        Page<User> pageUser = PageHelper.startPage(pageParamRequest.getPage(), pageParamRequest.getLimit());
        LambdaQueryWrapper<User> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        // 过滤机器人
        lambdaQueryWrapper.eq(User::getUserType, Constants.USER_LOGIN_TYPE_BOT);
        lambdaQueryWrapper.eq(request.getStatus() != null, User::getStatus, request.getStatus());
     ...
     */
    @Override
    public User randomGetOneBot() {
        return userDao.randomGetOneBot();
    }
    ...
    public Boolean updateIntegralMoney(UserOperateIntegralMoneyRequest request) {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (request.getIntegralValue() > 0) {
                ...
                if (request.getIntegralType() == 1) {// 增加
                    ...
                    integralRecord.setMark(StrUtil.format("后台操作增加了{}金豆", request.getIntegralValue()));
                    ...
                    integralRecord.setBalance(user.getIntegral() - request.getIntegralValue());
                    integralRecord.setMark(StrUtil.format("后台操作减少了{}金豆", request.getIntegralValue()));
        ...
        if (!execute) {
            throw new CrmebException("修改金豆/余额失败");
        }
        return execute;
    }
     ...
     */
    @Override
    public Boolean registerReset(PasswordRequest request) {
        //检测验证码
        if (StringUtils.isBlank(request.getValidateCode())) {
    ...
    public Map<Object, Object> getAddUserCountGroupDate(String date) {
        ...
        // 过滤机器人
        queryWrapper.ne("user_type", Constants.USER_LOGIN_TYPE_BOT);
    ...
    public PageInfo<User> getAdminSpreadPeopleList(String keywords, String dateLimit, PageParamRequest pageRequest) {
        ...
        // 过滤机器人
        lqw.ne(User::getUserType, Constants.USER_LOGIN_TYPE_BOT);
     ...
     *
     * @param userId           用户id
     * @param type             0=消费记录，1=金豆明细，2=签到记录，3=持有优惠券，4=余额变动，5=好友关系
     * @param pageParamRequest 分页参数
     * @return Object
     */
    @Override
...
//        Map<String, StoreOrder> orderMap = new HashMap<>();
//        orderList.forEach(e -> {
...
//    }
    ...
    public Boolean updateUser(UserUpdateRequest userRequest) {
        ...
        if (StringUtils.isNotBlank(userRequest.getPwd())) {
            user.setPwd(CrmebUtil.encryptPassword(userRequest.getPwd(), user.getPhone()));
        }

        // 更新用户银行卡信息
        LambdaUpdateWrapper<UserExtractInfo> update =  new LambdaUpdateWrapper<>();
        update.eq(UserExtractInfo::getUid, userRequest.getUid());
        UserExtractInfo userExtractInfo = new UserExtractInfo();
        userExtractInfo.setUid(userRequest.getUid());
        if(StringUtils.isNotEmpty(userRequest.getAccountName())){
            update.set(UserExtractInfo::getAccountName, userRequest.getAccountName());
            userExtractInfo.setAccountName(userRequest.getAccountName());
        }
        if(StringUtils.isNotEmpty(userRequest.getIcNumber())){
            update.set(UserExtractInfo::getIcNumber,  userRequest.getIcNumber());
            userExtractInfo.setIcNumber(userRequest.getIcNumber());
        }

        if(StringUtils.isNotEmpty(userRequest.getRealName())){
            update.set(UserExtractInfo::getRealName,  userRequest.getRealName());
            userExtractInfo.setRealName(userRequest.getRealName());
        }

        if(StringUtils.isNotEmpty(userRequest.getBankImage())){
            String img = systemAttachmentService.clearPrefix(userRequest.getBankImage());
            update.set(UserExtractInfo::getBankImage, img);
            userExtractInfo.setBankImage(img);
        }
        if(StringUtils.isNotEmpty(userRequest.getBankName())){
            update.set(UserExtractInfo::getBankName, userRequest.getBankName());
            userExtractInfo.setBankName(userRequest.getBankName());
        }
        if(StringUtils.isNotEmpty(userRequest.getAddress())){
            update.set(UserExtractInfo::getAddress, userRequest.getAddress());
            userExtractInfo.setAddress(userRequest.getAddress());
        }

        UserExtractInfo resUserExtractInfo = userExtractInfoService.getUserExtractInfoByUserId(userRequest.getUid());
        if(ObjectUtil.isEmpty(resUserExtractInfo)){
            userExtractInfo.setExtractType("bank");
            userExtractInfoService.save(userExtractInfo);

        }else{
            userExtractInfoService.update(update);
        }
     ...
     */
    @Override
    public Boolean updateBotUser(UserUpdateRequest userRequest) {
        User tempUser = getById(userRequest.getUid());
        if (!Constants.USER_LOGIN_TYPE_BOT.equals(tempUser.getUserType())) {
            throw new CrmebException("对应用户不是机器人");
        }
        User user = new User();
        if (StringUtils.isNotBlank(userRequest.getPwd())) {
            user.setPwd(CrmebUtil.encryptPassword(userRequest.getPwd(), user.getPhone()));
    ...
    public Integer getRegisterNumByPeriod(String startDate, String endDate) {
        ...
        wrapper.select("uid");
        // 过滤机器人
        wrapper.ne("user_type", Constants.USER_LOGIN_TYPE_BOT);
    ...
    public Boolean userLogoff() {
        User user = getInfo();
        if (0 != user.getNowMoney().compareTo(BigDecimal.ZERO)) {
            throw new CrmebException("请提现后在注销");
        }
        if (0 != user.getBrokeragePrice().compareTo(BigDecimal.ZERO)) {
            throw new CrmebException("请提现后在注销");
        }
        user.setStatus(false);
        user.setSpreadUid(0);
        user.setAccount("已注销" + user.getPhone() + "时间" + System.currentTimeMillis());
