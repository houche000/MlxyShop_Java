...
public class StoreCombinationServiceImpl extends ServiceImpl<StoreCombinationDao, StoreCombination> implements StoreCombinationService {
     ...
     */
    @Override
    public Integer saveCombination(StoreCombinationRequest request) {
        ...
        // 拼团分类
        storeCombination.setPinkCateId(request.getPinkCateId());
        // 活动开始结束时间
        ...
        StoreProductAttrValueAddRequest minKtAttrValue = attrValueAddRequestList.stream().min(Comparator.comparing(StoreProductAttrValueAddRequest::getKtPrice)).get();
        StoreProductAttrValueAddRequest minCtAttrValue = attrValueAddRequestList.stream().min(Comparator.comparing(StoreProductAttrValueAddRequest::getCtPrice)).get();
        storeCombination.setPrice(minAttrValue.getPrice());
        storeCombination.setKtPrice(minKtAttrValue.getKtPrice());
        storeCombination.setCtPrice(minCtAttrValue.getCtPrice());
        ...
        Integer storeCombinationId = transactionTemplate.execute(e -> {
            ...
            return storeCombination.getId();
        });

        return storeCombinationId;
    }
...
//        storeProductResponse.setStopTimeStr(DateUtil.timestamp2DateStr(storeCombination.getStopTime(), Constants.DATE_FORMAT_DATE));
     ...
     */
    @Override
    public List<StoreCombinationH5Response> getH5List(ProductRequest request, PageParamRequest pageParamRequest) {
        LambdaQueryWrapper<StoreCombination> lqw = Wrappers.lambdaQuery();
        lqw.select(StoreCombination::getId, StoreCombination::getSales, StoreCombination::getProductId, StoreCombination::getImage, StoreCombination::getTitle
                , StoreCombination::getTitleEn, StoreCombination::getTitleMy, StoreCombination::getPeople, StoreCombination::getOtPrice, StoreCombination::getPrice, StoreCombination::getKtPrice, StoreCombination::getStock);
        ...
        if (ObjectUtil.isNotNull(request.getCid()) && request.getCid() > 0) {
            ...
            lqw.apply(CrmebUtil.getFindInSetSql("pink_cate_id", (ArrayList<Integer>) categoryIdList));
        }
        if (StrUtil.isNotBlank(request.getKeyword())) {
            if (CrmebUtil.isString2Num(request.getKeyword())) {
                Integer productId = Integer.valueOf(request.getKeyword());
                lqw.like(StoreCombination::getId, productId);
            } else {
                lqw.like(StoreCombination::getTitle, request.getKeyword());
            }
        }
        // 排序部分
        if (StrUtil.isNotBlank(request.getSalesOrder())) {
            ...
            if (StrUtil.isNotBlank(request.getPriceOrder())) {
                if (request.getPriceOrder().equals(Constants.SORT_DESC)) {
                    lqw.orderByDesc(StoreCombination::getPrice);
                } else {
                    lqw.orderByAsc(StoreCombination::getPrice);
                }
            }

            lqw.orderByDesc(StoreCombination::getSort);
            lqw.orderByDesc(StoreCombination::getId);
        }
    ...
    public CombinationDetailResponse getH5Detail(Integer comId) {
        ...
        infoResponse.setStoreNameEn(storeCombination.getTitleEn());
        infoResponse.setStoreNameMy(storeCombination.getTitleMy());
        ...
        // 拼团结束时间
        infoResponse.setStopTime(storeCombination.getStopTime());
...
//        List<ProductAttrResponse> skuAttr = getSkuAttr(attrList);
        ...
        // 拼团团长列表
        List<StorePink> headList = storePinkService.getListByCidAndKid(storeCombination.getId(), 0, 10);
        if (CollUtil.isNotEmpty(headList)) {
            ...
            // 拼团成功部分
            LambdaQueryWrapper<StorePink> pinkOkWrapper = Wrappers.lambdaQuery();
            pinkOkWrapper.eq(StorePink::getCid, storeCombination.getId());
            pinkOkWrapper.eq(StorePink::getStatus, 2);
            pinkOkWrapper.eq(StorePink::getIsRefund, false);
            int pinkOkSum = storePinkService.count(pinkOkWrapper);
            detailResponse.setPinkOkSum(pinkOkSum);
            pinkOkWrapper.last("limit 10");
            List<StorePink> okPinkList = storePinkService.list(pinkOkWrapper);
            List<StorePinkResponse> okList = okPinkList.stream().map(i -> {
                StorePinkResponse pinkResponse = new StorePinkResponse();
                BeanUtils.copyProperties(i, pinkResponse);
                User teamUser = userService.getById(i.getUid());
                pinkResponse.setAvatar(Optional.ofNullable(teamUser.getAvatar()).orElse(""));
            ...
            List<StorePinkResponse> responseList = okList.stream().map(i -> {
                StorePinkResponse pinkResponse = new StorePinkResponse();
                BeanUtils.copyProperties(i, pinkResponse);
                User teamUser = userService.getById(i.getUid());
                pinkResponse.setAvatar(Optional.ofNullable(teamUser.getAvatar()).orElse(""));
     ...
     */
    @Override
    public CommonPage<StorePinkResponse> combineList(StorePinkSearchRequest request, PageParamRequest pageParamRequest) {
        // 拼团团长列表
        LambdaQueryWrapper<StorePink> lqw = new LambdaQueryWrapper<>();
        lqw.eq(StorePink::getCid, request.getId());
        if (StrUtil.isNotBlank(request.getKeyWord())) {
        ...
        PageHelper.startPage(pageParamRequest.getPage(), pageParamRequest.getLimit());
        List<StorePink> headList = storePinkService.list(lqw);
        CommonPage<StorePink> restPage = CommonPage.restPage(headList);
        ...
        if (CollUtil.isNotEmpty(headList)) {
            ...
            // 可拼团团长部分
            List<StorePinkResponse> pinkingList = headPinkList.stream().filter(i -> i.getStatus().equals(1) && i.getStopTime() > System.currentTimeMillis()).collect(Collectors.toList());
            // 获取还剩几人成团
            pinkingList.forEach(i -> {
                Integer countPeople = storePinkService.getCountByKid(i.getId());
                i.setCountPeople(countPeople);
            ...
            CommonPage<StorePinkResponse> responseCommonPage = CommonPage.restPage(headPinkList);
            BeanUtils.copyProperties(restPage, responseCommonPage, "list");
            return responseCommonPage;
        } else {
    ...
    public GoPinkResponse goPink(Integer pinkId) {
        ...
        if (ObjectUtil.isNull(teamPink)) throw new CrmebException("对应的拼团不存在");
        ...
        if (teamPink.getKId().equals(0)) {
            pinkList = storePinkService.getListByCidAndKid(teamPink.getCid(), teamPink.getId(), 10);
            pinkList.add(teamPink);
        } else {
            pinkList = storePinkService.getListByCidAndKid(teamPink.getCid(), teamPink.getKId(), null);
    ...
    public Boolean removePink(StorePinkRequest storePinkRequest) {
        ...
        refundRequest.setAmount(order.getPayPrice().subtract(order.getRefundPrice()));
        boolean apply = storeOrderService.refund(refundRequest);
        if (!apply) throw new CrmebException("订单退款失败");

        if (userPink.getKId() == 0) {// 是团长
            List<StorePink> pinkList = storePinkService.getListByCidAndKid(userPink.getCid(), userPink.getId(), null);
            if (CollUtil.isNotEmpty(pinkList)) {
                // 团员更换团长
        ...
        // 拼团改为未完成
        userPink.setKId(0).setStatus(3).setStopTime(System.currentTimeMillis());
    ...
    @Override
    public Boolean isPink(Integer type) {
        LambdaQueryWrapper<StorePink> countWrapper = new LambdaQueryWrapper<>();
        LocalDate now = LocalDate.now();
        countWrapper.between(StorePink::getAddTime, now.atStartOfDay(GMT).toEpochSecond() * 1000, now.plusDays(1L).atStartOfDay(GMT).toEpochSecond() * 1000);
        countWrapper.eq(StorePink::getUid, userService.getUserId());
        countWrapper.ne(StorePink::getStatus, 3);
        if (type == 1) {
            // 每日最大开团次数
            String startMaxString = systemConfigService.getValueByKey(SysConfigConstants.DAILY_GROUP_START_MAX);
            if (StrUtil.isNotEmpty(startMaxString)) {
                int startMax = Integer.parseInt(startMaxString);
                if (startMax > 0) {
                    countWrapper.eq(StorePink::getKId, 0);
                    int count = storePinkService.count(countWrapper);
                    if (count >= startMax) {
                        return false;
                    }
                }
            ...
            // 每日最大参团次数
            String partMaxString = systemConfigService.getValueByKey(SysConfigConstants.DAILY_GROUP_MAX);
            if (StrUtil.isNotEmpty(partMaxString)) {
                int partMax = Integer.parseInt(partMaxString);
                if (partMax > 0) {
                    countWrapper.ne(StorePink::getKId, 0);
                    int count = storePinkService.count(countWrapper);
                    if (count >= partMax) {
                        return false;
                    }
