    ...
    public void autoStatistics() {
       ...
       //  ========================================
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            // 防止重复插入
            LambdaQueryWrapper<TradingDayRecord> dayQueryWrapper = Wrappers.lambdaQuery();
            dayQueryWrapper.eq(TradingDayRecord::getDate, tradingDayRecord.getDate());
            TradingDayRecord record = tradingDayRecordService.getOne(dayQueryWrapper);
            if (record != null) {
                tradingDayRecord.setId(record.getId());
                tradingDayRecordService.saveOrUpdate(tradingDayRecord);
            }
            return Boolean.TRUE;
        });
        if (!execute) {
