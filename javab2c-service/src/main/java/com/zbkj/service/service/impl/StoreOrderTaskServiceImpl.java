...
//                    stockRequest.setAttrId(orderInfoVo.getAttrValueId());
//                    stockRequest.setOperationType("add");
//                    stockRequest.setType(1);
//                    stockRequest.setSuk(orderInfoVo.getSku());
//                    if(orderInfoVo.getPayNum() < 1){
//                        //如果取不到值，则跳过
//                        continue;
//                    }
//                    stockRequest.setNum(orderInfoVo.getPayNum());
...
//            throw new CrmebException(e.getMessage());
    ...
    public Boolean refundOrder(StoreOrder storeOrder) {
        ...
        integralRecordList.forEach(record -> {
            if (record.getType().equals(IntegralRecordConstants.INTEGRAL_RECORD_TYPE_SUB)) {// 订单抵扣部分
                ...
                record.setMark(StrUtil.format("订单退款，返还支付扣除得{}金豆", record.getIntegral()));
            ...
            } else if (record.getType().equals(IntegralRecordConstants.INTEGRAL_RECORD_TYPE_ADD)) {// 冻结金豆部分
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (userExperienceRecord != null) {
                ...
                try {
                    ...
                    logger.error("订单处理,等级降级异常", levelEx);
                }
            }
            ...
            } catch (Exception rollbackStockEx) {
                logger.error("订单处理,回滚库存异常", rollbackStockEx);
            }
            ...
            if (storeOrder.getCombinationId() > 0) {
                ...
                // 只修改进行中的拼团状态为失败
                if (storePink.getStatus().equals(1)) {
                    storePink.setStatus(3);
                }
                storePink.setIsRefund(true).setStopTime(System.currentTimeMillis());
    ...
    public Boolean orderReceiving(Integer orderId) {
        ...
        // 获取金豆记录
        List<UserIntegralRecord> integralRecordList = userIntegralRecordService.findListByOrderIdAndUid(storeOrder.getOrderId(), storeOrder.getUid());
        logger.info("收货处理金豆条数：" + integralRecordList.size());
        ...
        for (UserIntegralRecord record : userIntegralRecordList) {
            if (!record.getStatus().equals(IntegralRecordConstants.INTEGRAL_RECORD_STATUS_CREATE)) {
                throw new CrmebException(StrUtil.format("订单收货task处理，订单金豆记录不是创建状态，id={}", orderId));
            }
            // 佣金进入冻结期
