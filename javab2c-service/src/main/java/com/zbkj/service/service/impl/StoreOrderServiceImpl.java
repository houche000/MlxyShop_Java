    ...
    */
    ...
    public CommonPage<StoreOrderDetailResponse> getAdminList(StoreOrderSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        queryWrapper.select("id", "order_id", "uid", "real_name", "pay_price", "pay_type", "create_time", "status", "refund_status"
                ...
                , "is_del", "combination_id", "pink_id", "seckill_id","presale_id", "bargain_id", "verify_code", "remark", "paid", "is_system_del", "shipping_type", "type", "is_alter_price", "user_address", "user_phone","postal_code");
        ...
        if (StrUtil.isNotBlank(request.getOutTradeNo())) {
            queryWrapper.eq("out_trade_no", request.getOutTradeNo());
        }
        ...
        if (!request.getType().equals(2)) {
            if (request.getType().equals(3)) {
                queryWrapper.and(storeOrderQueryWrapper -> {
                    storeOrderQueryWrapper.isNotNull("combination_id");
                    storeOrderQueryWrapper.gt("combination_id", 0);
                });
            } else if (request.getType().equals(4)) {
                queryWrapper.and(storeOrderQueryWrapper -> {
                    storeOrderQueryWrapper.isNotNull("seckill_id");
                    storeOrderQueryWrapper.gt("seckill_id", 0);
                });
                ...
                queryWrapper.inSql("order_id", "select order_id from eb_store_pink where uid = winner");
            }else if (request.getType().equals(6)) {
                queryWrapper.and(storeOrderQueryWrapper -> {
                    storeOrderQueryWrapper.isNotNull("presale_id");
                    storeOrderQueryWrapper.gt("presale_id", 0);
                });
            } else {
            ...
            queryWrapper.and(storeOrderQueryWrapper -> {
                storeOrderQueryWrapper.isNull("combination_id")
                        .or().eq("combination_id", 0);
            });
            queryWrapper.and(storeOrderQueryWrapper -> {
                storeOrderQueryWrapper.isNull("verify_code").or().eq("verify_code", "");
            });
            queryWrapper.and(storeOrderQueryWrapper -> {
                storeOrderQueryWrapper.isNull("seckill_id")
                        .or().eq("seckill_id", 0);
            });
            queryWrapper.and(storeOrderQueryWrapper -> {
                storeOrderQueryWrapper.isNull("bargain_id")
                        .or().eq("bargain_id", 0);
            });
            queryWrapper.and(storeOrderQueryWrapper -> {
                storeOrderQueryWrapper.isNull("presale_id")
                        .or().eq("presale_id", 0);
            });
        }
        // 过滤没有中奖的订单
        if (!request.getStatus().equals("refunding") && !request.getStatus().equals("refunded")) {
            queryWrapper.notInSql("order_id", "SELECT order_id FROM eb_store_pink WHERE order_id != '0' AND `status` != 2 AND (winner IS NULL OR uid != winner)");
        }
    ...
    public List<StoreOrder> getUserOrderList(Integer uid, Integer status, PageParamRequest pageParamRequest) {
        ...
        //过滤待发货拼团进行中订单
        if (status == 1) {
            lqw.notInSql(StoreOrder::getOrderId, "select order_id from eb_store_pink where uid = " + uid + " and status = 1");
        }
        return dao.selectList(lqw);
    }

    @Override
    public List<StorePink> getUserCombinationOrderList(Integer uid, Integer status, Boolean isWinner, PageParamRequest pageParamRequest) {
        PageHelper.startPage(pageParamRequest.getPage(), pageParamRequest.getLimit());
        LambdaQueryWrapper<StorePink> lqw = new LambdaQueryWrapper<>();
        lqw.eq(StorePink::getStatus, status);
        lqw.eq(StorePink::getUid, uid);
        if (isWinner != null) {
            if (isWinner) {
                lqw.eq(StorePink::getWinner, uid);
            } else {
                lqw.ne(StorePink::getWinner, uid);
            }
        }
        lqw.orderByDesc(StorePink::getId);
        List<StorePink> storePinkList = storePinkService.list(lqw);
        return storePinkList;
    }
...
//        HashMap<Integer, User> userList = userService.getMapListInUid(userIdList);
        ...
        for (StoreOrder storeOrder : orderList) {
            ...
            if (storeOrder.getPinkId() > 0) {
                ...
                storeOrderItemResponse.setPinkInfo(storePink);
            }
            ...
            storeOrderItemResponse.setUserPhone(storeOrder.getUserPhone());
            storeOrderItemResponse.setPostalCode(storeOrder.getPostalCode());

            storeOrderItemResponse.setUserAddress(storeOrder.getUserAddress());
            // 添加订单类型信息
    ...
    private String getOrderTypeStr(StoreOrder storeOrder) {
        ...
        // 预售
        if (ObjectUtil.isNotNull(storeOrder.getPresaleId()) && storeOrder.getPresaleId() > 0) {
            orderType = StrUtil.format(orderTypeFormat, "预售", "");
        }
        // 拼团
...
//        HashMap<Integer, SystemStoreStaff> systemStoreStaffList = systemStoreStaffService.getMapInId(clerkIdList);
    ...
    public boolean refund(StoreOrderRefundRequest request) {
        ...
        // 是否原路退款判断
        String refundTypeString = systemConfigService.getValueByKey(SysConfigConstants.REFUND_CONFIG_TYPE);
        int refundType = Integer.parseInt(refundTypeString);
        String payType = storeOrder.getPayType();
        if (Constants.REFUND_TO_GO.equals(refundType)) {
            //退款
            if (payType.equals(Constants.PAY_TYPE_WE_CHAT) && request.getAmount().compareTo(BigDecimal.ZERO) > 0) {
                try {
            ...
            if (payType.equals(Constants.PAY_TYPE_ALI_PAY) && request.getAmount().compareTo(BigDecimal.ZERO) > 0) {
                try {
            ...
            if (payType.equals(Constants.PAY_TYPE_YY_PAY) && request.getAmount().compareTo(BigDecimal.ZERO) > 0) {
                try {
                    yyPayService.refund(request, storeOrder);
                } catch (Exception e) {
                    e.printStackTrace();
                    throw new CrmebException("盈富申请退款失败！");
                }
            }
            if (payType.equals(Constants.PAY_TYPE_ADA_PAY) && request.getAmount().compareTo(BigDecimal.ZERO) > 0) {
                try {
                    adaPayService.refund(request, storeOrder);
                } catch (Exception e) {
                    e.printStackTrace();
                    throw new CrmebException("ada申请退款失败！");
                }
            }
        }
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            // 是否退款到余额判断
            if (payType.equals(Constants.PAY_TYPE_YUE) || payType.equals(Constants.PAY_TYPE_RAZER_PAY) || payType.equals(Constants.PAY_TYPE_DURIAN_PAY) || Constants.REFUND_TO_YUE.equals(refundType)) {
                //新增日志
                request.setOrderId(storeOrder.getId());
                // 更新用户金额
...
//        pushMessageRefundOrder(storeOrder, user, temMap);
    ...
    @Override
    public boolean refundYue(String oredrNo) {
        StoreOrder storeOrder = getInfoException(oredrNo);
        //修改订单退款状态
        storeOrder.setRefundStatus(3);
        storeOrder.setRefundPrice(storeOrder.getPayPrice());
        StoreOrderRefundRequest request = new StoreOrderRefundRequest();
        request.setAmount(storeOrder.getPayPrice());
        request.setOrderNo(oredrNo);
        request.setOrderId(storeOrder.getId());
        User user = userService.getById(storeOrder.getUid());
        Boolean execute = transactionTemplate.execute(e -> {
            updateById(storeOrder);
...
//        if (storeOrder.getStatus() > 0) throw new CrmebException("订单已发货请勿重复操作!");
     ...
     */
    @Override
    public LogisticsResultVo getLogisticsInfo(String orderNo, String type) {
        ...
        return logisticService.info(info.getDeliveryId(), type, Optional.ofNullable(info.getDeliveryCode()).orElse(""), info.getUserPhone());
    }
    ...
    public Integer getTopDataUtil(Integer status, Integer userId) {
        ...
        //过滤待发货拼团进行中订单
        if (status == 1) {
            lqw.notInSql(StoreOrder::getOrderId, "select order_id from eb_store_pink where uid = " + userId + " and status = 1");
        }
     ...
     */
    @Override
    public List<StoreOrder> getUserCurrentPresaleOrders(Integer uid, Integer presaleId) {
        LambdaQueryWrapper<StoreOrder> lqw = Wrappers.lambdaQuery();
        lqw.eq(StoreOrder::getUid, uid);
        lqw.eq(StoreOrder::getPresaleId, presaleId);
        lqw.eq(StoreOrder::getIsDel, false);
        return dao.selectList(lqw);
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////////// 以下为自定义方法
    ...
    private void express(StoreOrderSendRequest request, StoreOrder storeOrder) {
        ...
        if (express == null) {
            throw new CrmebException("快递公司不存在，请同步快递公司！");
        }
...
//        map.put("phone_number10", request.getDeliveryTel());
    ...
    private Integer getCount(String dateLimit, String status, Integer type) {
        ...
        if (Integer.valueOf(3).equals(type)) {
            ...
            queryWrapper.eq("combination_id", 0);
        }

        // 过滤没有中奖的订单
        if (!status.equals("refunding") && !status.equals("refunded")) {
            queryWrapper.notInSql("order_id", "SELECT order_id FROM eb_store_pink WHERE order_id != '0' AND `status` != 2 AND (winner IS NULL OR uid != winner)");
        }
