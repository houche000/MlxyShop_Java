...
public class ScheduleManager {
     ...
     */
    public List<Integer> getAllJobIds() throws SchedulerException {
        Set<JobKey> keySet = scheduler.getJobKeys(GroupMatcher.anyJobGroup());
        return keySet.stream().map(jobKey -> Integer.parseInt(jobKey.getName().replace(JOB_NAME, ""))).collect(Collectors.toList());
    }
     ...
     * 删除定时任务
     */
    public void deleteScheduleJobById(Integer id) {
        try {
            // 停止触发器
            ...
            //删除任务
            scheduler.deleteJob(JobKey.jobKey(JOB_NAME + id));
        } catch (SchedulerException e) {
            throw new RuntimeException("删除定时任务失败", e);
        }
    }
