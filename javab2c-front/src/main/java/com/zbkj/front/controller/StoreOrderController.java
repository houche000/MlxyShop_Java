...
public class StoreOrderController {
    ...
    @RequestMapping(value = "/combinationOrderList", method = RequestMethod.GET)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "isWinner", value = "null=全部,true=中奖,false=没中奖", required = true),
            @ApiImplicitParam(name = "type", value = "拼团状态，1=拼团中,2=拼团成功,3=拼团失败", required = true)
    })
    public CommonResult<CommonPage<StorePinkResponse>> combinationOrderList(@RequestParam(name = "type") Integer type,
                                                                            @RequestParam(name = "isWinner") Boolean isWinner,
                                                                            @ModelAttribute PageParamRequest pageRequest) {
        return CommonResult.success(orderService.combinationOrderList(type, isWinner, pageRequest));
    }
     ...
     * @return 订单列表
     */
    @ApiOperation(value = "拼团订单统计")
    @RequestMapping(value = "/combinationOrderCount", method = RequestMethod.GET)
    public CommonResult<List<Map<String, Object>>> combinationOrderCount() {
        return CommonResult.success(orderService.combinationOrderCount());
    }
    ...
    @RequestMapping(value = "/express/{orderId}/{type}", method = RequestMethod.GET)
    public CommonResult<Object> getExpressInfo(@PathVariable("orderId") String orderId, @PathVariable(value = "type", required = false) String type) {
        return CommonResult.success(orderService.expressOrder(orderId, type));
    }
