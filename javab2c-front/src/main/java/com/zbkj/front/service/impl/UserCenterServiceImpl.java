...
public class UserCenterServiceImpl extends ServiceImpl<UserDao, User> implements UserCenterService {
     ...
     */
    @Override
    public UserCommissionResponse getCommission(Integer type) {
        User user = userService.getInfoException();
        // 昨天得佣金
        BigDecimal yesterdayIncomes = userBrokerageRecordService.getYesterdayIncomes(user.getUid());
        //累计已提取佣金
        BigDecimal totalMoney = userExtractService.getExtractTotalMoney(user.getUid(), type);
    ...
    public BigDecimal getSpreadCountByType(Integer type) {
        ...
        if (type == 3) {
            BigDecimal count = userBillService.getSumBigDecimal(null, userId, Constants.USER_BILL_CATEGORY_MONEY, null, Constants.USER_BILL_TYPE_BROKERAGE, null);
            BigDecimal withdraw = userBillService.getSumBigDecimal(1, userId, Constants.USER_BILL_CATEGORY_MONEY, null, Constants.USER_BILL_TYPE_BROKERAGE, null); //提现
            return count.subtract(withdraw);
        }

        //累计提现
        if (type == 4) {
            return userExtractService.getWithdrawn(null, null, EnumFundType.COMMISSION.getCode());
        }
    ...
    public UserBalanceResponse getUserBalance() {
        ...
        BigDecimal recharge = userBillService.getSumBigDecimal(1, info.getUid(), Constants.USER_BILL_CATEGORY_MONEY, null, null, null);
        BigDecimal orderStatusSum = userBillService.getSumBigDecimal(0, info.getUid(), Constants.USER_BILL_CATEGORY_MONEY, null, null, "extract");
        BigDecimal extractSum = userBillService.getSumBigDecimal(0, info.getUid(), Constants.USER_BILL_CATEGORY_MONEY, null, "extract", null);
//        BigDecimal orderStatusSum = storeOrderService.getSumBigDecimal(info.getUid(), null);
        return new UserBalanceResponse(info.getNowMoney(), recharge, orderStatusSum, extractSum);
    }
     ...
     */
    @Override
    public PageInfo<UserExtractRecordResponse> getExtractRecord(Integer type, PageParamRequest pageParamRequest) {
        Integer userId = userService.getUserIdException();
        return userExtractService.getExtractRecord(userId, type, pageParamRequest);
    }
     ...
     * 用户账单记录（现金）
     *
     * @param type 记录类型：all-全部，expenditure-支出，income-收入，extract，-提现
     * @return CommonPage
     */
    @Override
    public CommonPage<UserRechargeBillRecordResponse> nowMoneyBillRecord(String type, PageParamRequest pageRequest) {
        ...
        responseList.sort((o1, o2) -> {
            String o1Date = o1.getDate().replace("-", "");
            String o2Date = o2.getDate().replace("-", "");
            return Integer.parseInt(o2Date) - Integer.parseInt(o1Date);
        });
     ...
     */
    @Override
    public UserExtractCashResponse getExtractUser(Integer type) {
        User user = userService.getInfoException();
        // 提现最低金额
        ...
        if (EnumFundType.BALANCE.getCode().equals(type)) {
            ...
            // 可提现佣金
            BigDecimal brokeragePrice = user.getNowMoney();
            return new UserExtractCashResponse(minPrice, brokeragePrice, BigDecimal.ZERO, extractTime);
        }
        // 冻结天数
        String extractTime = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_EXTRACT_FREEZING_TIME);
        // 可提现佣金
...
//        String encode;
     ...
     */
    @Override
    public OrderPayResultResponse razerRecharge(UserRechargeRequest request) {
        request.setPayType(Constants.PAY_TYPE_RAZER_PAY);
        ...
        response.setPayType(Constants.PAY_TYPE_RAZER_PAY);
        response.setOrderNo(userRecharge.getOrderId());
        String vcode = RazerPayUtils.generateKeys(request.getPrice().toString(), RazerPayUtils.mpsmerchantid, userRecharge.getOrderId(), RazerPayUtils.vkey);
    ...
    @Override
    public OrderPayResultResponse durianRecharge(UserRechargeRequest request) {
        request.setPayType(Constants.PAY_TYPE_DURIAN_PAY);
        ...
        OrderPayResultResponse response = new OrderPayResultResponse();
        response.setStatus(false);
        response.setPayType(Constants.PAY_TYPE_DURIAN_PAY);
        response.setOrderNo(userRecharge.getOrderId());
        ...
        map.put("signature", DurianPayConfig.getSignature(merchantCode, secretKey, userRecharge.getOrderId(), "MYR", request.getPrice().toString()));
        response.setDurianPayRequest(map);
        return response;
    }

    @Override
    public OrderPayResultResponse adaRecharge(UserRechargeRequest request) {
        request.setPayType(Constants.PAY_TYPE_ADA_PAY);
        ...
        paymentParams.put("order_no", userRecharge.getOrderId());
        paymentParams.put("pay_channel", request.getFromType());
        paymentParams.put("pay_amt", userRecharge.getPrice().setScale(2, RoundingMode.HALF_UP).toString());
        paymentParams.put("goods_title", "充值：" + userRecharge.getPrice() + "元");
        paymentParams.put("goods_desc", "充值：" + userRecharge.getPrice() + "元");
        paymentParams.put("notify_url", systemConfigService.getValueByKey(AdaPayConstants.ADA_CALLBACK_URL));
        if ("wx_pub".equals(request.getFromType()) || "wx_lite".equals(request.getFromType())) {
            User user = userService.getInfo();
