...
//                    }
    ...
    /**
     * 是否在当前时间段内
     * @param time {"01:00","23:00"}
     * @return
     */
    public static Boolean hourBetween(String[] time) {
        int startTime = Integer.parseInt(time[0].replace(":", ""));
        int endTime =  Integer.parseInt(time[1].replace(":",""));
        int hour = Calendar.getInstance().get(Calendar.HOUR_OF_DAY);
